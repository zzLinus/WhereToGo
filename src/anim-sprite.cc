...
void AnimSprite::anim_finished_callback()
{
  m_isFinished = false;
}

void AnimSprite::upload_drawable(Vector2 position, bool invert)
{
  float w;
  if (invert)
    w = -m_frameRect.width;
  ...
  RenderObject* rdobj = new RenderObject(
      m_sprite.texture,
      ...
      WHITE,
      TEXTUREPRO);
  p_renderer->add_renderObj(rdobj, RENDER_2D_MOVER);
  delete rdobj;
}
