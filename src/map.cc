  ...
  for (auto& layer : layers)
  {
    if (layer.getType() == tson::LayerType::ObjectGroup)
    {
      for (auto& obj : layer.getObjects())
      {
        ...
        {
          tson::Rect objRect = Rect(obj.getPosition().x, obj.getPosition().y, obj.getSize().x, obj.getSize().y);
...
void TileMap::check_layer(tson::Layer& layer, RaylibTilesonData* data, int posX, int posY, Color tint)
{
  ...
  {
    case tson::LayerType::TileLayer: upload_tileLayer(layer, data, posX, posY, tint); break;

    case tson::LayerType::ObjectGroup: uplaod_objLayer(layer, data, posX, posY, tint); break;

    case tson::LayerType::ImageLayer: upload_imgLayer(layer, data, posX, posY, tint); break;
    ...
    default: TraceLog(LOG_TRACE, "TILESON: Unsupported layer type"); break;
  }
}
