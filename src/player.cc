...
void Player::handle_keyboard()
{
    ...
    if (IsKeyDown(KEY_N) && !(m_lastState == S_ATTACK1 && m_currentFrame != 4)) {
        m_currentState = S_ATTACK1;
        m_current_weapon_state = B_ATTACK1;
        m_currentFrame = 0;
    } else if (IsKeyDown(KEY_S) && !(m_lastState == S_ATTACK1 && m_currentFrame != 4)) {
        m_is_left = false;
        m_currentState = S_RUN;
    } else if (IsKeyDown(KEY_A) && !(m_lastState == S_ATTACK1 && m_currentFrame != 4)) {
        m_is_left = true;
        m_currentState = S_RUN;
    } else if (IsKeyDown(KEY_W) && !(m_lastState == S_ATTACK1 && m_currentFrame != 4)) {
        m_currentState = S_RUN_UP;
    } else if (IsKeyDown(KEY_R) && !(m_lastState == S_ATTACK1 && m_currentFrame != 4)) {
...
void Player::Render()
{
    ...
    if (m_frameCounter >= (60 / m_frameSpeed)) {
        ...
        char_weapon_blade[B_ATTACK1].frameRec.x
