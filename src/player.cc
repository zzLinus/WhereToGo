...
void Player::handle_keyboard()
{
  ...
  m_position = Vector2Add(
      m_position,
      Vector2Multiply(Vector2Normalize(Vector2{ direction.x, direction.y }), Vector2{ .x = m_speed, .y = m_speed }));
...
void Player::update()
{
  ...
  m_collisionBox.x =
      m_position.x +
      (float)m_animSprites[m_curState].m_sprite.texture.width / (2 * (float)m_animSprites[m_curState].m_sprite.frameNum) - 7;
...
void Player::upload_hud_drawobj()
{
  ...
  if (debugInfo == DebugInfo::enable)
  {
    ...
    rdobj = new RenderObject(
        15,
        40,
        m_animSprites[m_curState].m_sprite.texture.width,
        m_animSprites[m_curState].m_sprite.texture.height,
        LIME,
        RECTLINE);
    p_renderer->add_renderObj(rdobj, RENDER_NOR);
    delete rdobj;
    ...
    rdobj = new RenderObject(
        15 + (int)m_animSprites[m_curState].m_frameRect.x,
        40 + (int)m_animSprites[m_curState].m_frameRect.y,
        (int)m_animSprites[m_curState].m_frameRect.width,
        (int)m_animSprites[m_curState].m_frameRect.height,
        RED,
        RECTLINE);
    p_renderer->add_renderObj(rdobj, RENDER_NOR);
    delete rdobj;
...
void Player::upload_drawable()
{
  ...
  if (m_isLeft)
    tmpVect2 =
        Vector2Add(m_position, Vector2{ .x = -m_bladeOffset[m_curWeaponState].x, .y = m_bladeOffset[m_curWeaponState].y });
  else
